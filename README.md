...
- **v3.5.29:** Mieru rows in 实例出口管理 (+ status list; list via db after Xray; never add mieru to XRAY_PROTOCOLS). Routing **templates** (matchers-only) vs **profiles**; wizard/picker creates/binds 家宽 + 直出备用 **via 实例出口管理** (picker items 家宽 / 直出备用 / 配置重建 wizard; stable ids `home`/`direct_backup` — Chinese UI-only). No generic top-level profile CRUD menu; no auto-seeding finance_crypto/telegram_dc/ai_media as profiles (those stay templates). Self-updater SoT: configured `SCRIPT_SOURCE_REPO`/`REF`/`PATH` (manual 脚本更新 ignores 1h cache; not stale tag/release).
- **v3.5.28:** Per-instance **routing profiles** (`profile:<id>`) — matcher packs `ai_media` / `finance_crypto` / `telegram_dc` (matchers + Telegram fallback; no fake `geoip:telegram`). **As of v3.5.29:** those packs are templates (matchers-only), not auto-profiles; top-level UI「分流规则集」/ profile CRUD **withdrawn** — bind 家宽 / 直出备用 (and 配置重建 wizard) via **实例出口管理** picker only; shared-profile smart-apply backend kept
